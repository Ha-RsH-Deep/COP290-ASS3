...
def books():
    ...
    if query:
        cursor.execute("""
            SELECT id, title, authors, rating, image
            FROM book
    ...
    books = books[start:end]
    return render_template("books.html", books=books , pagination=pagination, user=current_user, total_pages=total_pages , page=page)
